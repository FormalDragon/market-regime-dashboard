...
def render_overview_tab_sections(
    display_regime: str,
    dominant_signal: str,
    insight: str,
    market_patterns: list[dict],
    overview_metrics: dict,
    notable_assets: pd.DataFrame,
    score_cards_df: pd.DataFrame,
    score_history: pd.DataFrame,
    regime_display: dict[str, str],
):
    """Render only the overview-tab core sections in fixed order.

    NOTE:
    - Global/regional alignment is intentionally excluded here.
    - Confidence rendering is intentionally excluded here.
    """
    ...
    st.caption("최근 30일 변화율 또는 120일 기준 이례성 점수를 바탕으로 현재 시장에서 크게 반응한 자산을 선별합니다.")
    ...
    render_regime_score_history(score_history, regime_display)
...
def build_global_regional_alignment(regime: str, country_df: pd.DataFrame) -> dict:
    ...
    if regime == "Risk-Off":
        if falling / total >= 0.6:
            ...
            description = f"위험 회피 분류 조건이 충족된 상태이며, 주요 국가 {total}개 중 {falling}개가 하락으로 분류되어 지역 반응도 동일 방향으로 관찰됩니다."
        else:
            label = "글로벌 신호와 지역 반응 일부 불일치"
            description = f"위험 회피 분류 조건은 충족되었지만, 국가별 반응이 상승 {rising}개, 보합 {neutral}개, 하락 {falling}개로 분산되어 있습니다."
    elif regime == "Risk-On":
        if rising / total >= 0.6:
            label = "글로벌 신호와 지역 반응 일치"
            description = f"위험 선호 분류 조건이 충족된 상태이며, 주요 국가 {total}개 중 {rising}개가 상승으로 분류되어 지역 반응도 동일 방향으로 관찰됩니다."
        else:
            label = "글로벌 신호와 지역 반응 일부 불일치"
            description = f"위험 선호 분류 조건은 충족되었지만, 국가별 반응이 상승 {rising}개, 보합 {neutral}개, 하락 {falling}개로 분산되어 있습니다."
    elif regime == "Tightening Stress":
        label = "금리 압박 신호와 지역 반응 확인 필요"
        description = f"금리 압박 분류 조건이 충족되었습니다. 국가별 반응은 상승 {rising}개, 보합 {neutral}개, 하락 {falling}개입니다."
    else:
        label = "방향성 불명확"
        description = f"주요 상태의 점수 우위가 제한되어 방향성 불명확으로 분류되었습니다. 국가별 반응은 상승 {rising}개, 보합 {neutral}개, 하락 {falling}개입니다."
